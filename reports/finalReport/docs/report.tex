...
Approach to traffic modelling originated as one following cellular automata model, with the map represented as grid consisting of cells maybe occupied by a car. Shortly after initial prototype implementation the requirement for real map data was raised. The cellular automata model was found to be too rough and inflexible inflexible due to fixed cell size. Such model did not pair well with the multitude of forms and combinations found in real maps, so it had to be abandoned in favour of, or rather morphed into, more physical metric model. In this physical model vehicle bears real-life properties such as speed, acceleration, angle and position on the lane, and moves according to laws of mechanics, which uncovers a potential for further detailisation of the physical model. Junctions had inspection properties attached to them, namely vehicle counters, making the software a tool for inspection. Data import from Open Street Map format was added, which allows for interoperability with a vast tooling and data ecosystem already available, as well as convenience in creating a synthetic test examples.

Vehicle behaviour side - driving and reasoning about - received less attention than originally anticipated. At the same time some interesting driving behaviour, such as acceleration and breaking taking road situation into account, turn decisions,  as well as lane changing, was implemented.

A sophisticated user interface has been created, partly to facilitate need for visual inspection of various map data, but also useful for actual simulation runs. Example of main application window is shown in Figure \ref{fig:screenshot}
...
During the course of implementation, accent was shifted from creating complex behavioural models for vehicles such as routing and implementing them as intelligent agents, to a different area of complexity: understanding, representing and displaying real world map data in a format required for traffic simulation. Still, vehicles operate in a rather autonomous ways, but they can not be classified as agents, since interface between environment and vehicle is not that well defined, and really cars are only capable of basic reactive behaviour.
...
Naturally, implementation of some features took place earlier, defining ongoing possibilities for other features to be implemented. As such, basic feature of crossing being free-form demanded some test definitions of different types of crossings, which in turn led to requirement for loading external data files. At the same time, GUI started to serve as a sort of visual inspector and debugger to understand data encounters, which led to a more sophisticated GUI design.

Team capacitly played not the last role in requirement adaptation as well. Defined features were distributed among team members and due to different level of efficiency some of them were implemented to a higher level than others.
...
While the test driven development process was selected by team initially, usage of proper unit testing has declined over time due to approaching deadlines. Still, tests were written as a regular practice for some parts of the system, especially illustrating more complex concepts or acting as early clients for the code to be integrated later.
...
Build system using Gradle was plugged in to automate builds and various artefact production. In order to work with OpenStreetMap files, appropriate standard OSM tools were used (namely, JOSM and osmosis), as detailed below in the report.
...
We could have improved our program by adding Traffic lights module. The figure below shows the prepared class diagram. All management should have been in traffic light controller class. The rules for changing traffic lifht's states were suppose to be in TrafficLightRule and TrafficLightsSetRule classes.
...
\begin{itemize}
    ...
    \item Traffic lights in one junction set would have the same ID as Juntion.
...
One particular point of interest would be the movement along the lanes. Currently, as the offset construction of the individual lanes within a road is made based on the polyline, a vehicle when travelling on the backward lane requires a calculation that subtract from the total length whereas, on the forward direction, the calculation adds from 0. The latter being more natural.

Certain planned functionalities were not achieved in the end as the work input capacity was not as originally anticipated and the creation of the graph from OSM data was more involved than previously thought. In the end most of the basic and advanced items planned were completed in addition of some extra ones (see Table~\ref{table:functionalities}).
...
\begin{itemize}
	...
	\item There is a set  number of cars that are produced for the simulation purpose (currently 500), but in general we could have improved it by creating a vehicle generator that depends on the map size.
...
We could have also developed additional options.
...
There could have been improvement in the logger.

\underline{Logger}
\begin{itemize}
	\item Logger might have coloured the output callable from the GUI.
	\item Logger's messages might have been filtered, basing on the message's origin or granular individual log level control.
\end{itemize}
