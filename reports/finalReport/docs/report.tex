...
AIMSUN is another popular and well studied, but commercial transport simulation software made by Transport Simulation Systems in Spain.[main site]
It is commonly classified as a microscopic based simulator. Nevertheless it alsso supports simulation of macro and meso models.
Depending on which model the user in intended to simulate he/she can configure spesific mode and manage the traffic flow. [nz].
Supporting a dynamic simulation AIMSUN can operate with larger road networks as highways, freeways, and ring roads.
By default it has simulation scenarios which allows user to run experiments with spesific parameters.
