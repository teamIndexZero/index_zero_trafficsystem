...
\begin{itemize}
    ...
    \end{lstlisting}
    \item Extensive caching of smaller helper object (java.awt.Stroke, java.awt.Color). Since most of the drawing is using line widhts dependent on the zoom level, it is beneficial to store static objects such as colors and stroke styles in caching maps for future re-use.    \begin{lstlisting}[language=java]
...
On every simulation tick, the state, including position, of the vehicles is re-calculated, and new frame is drawn. At the same time
\begin{itemize}
...
\begin{codeCaption}
	\caption{Access to the logger}
	\label{code:loggerAccess}
	\centering
	\begin{lstlisting}[language=java]
...
Since in Java most types are children of the Object class and we have 'autoboxing'\cite{Oracle1995} for primitive types we can use <Object> as the type for the signature of the method that are called to pass a message. In logging messages there are no set number of arguments for the description so there are two options opened to us: Allow strings only or allow any number of arguments to be passed to the methods for the log calls. The latter was chosen for flexibility's sake in order to be able to pass dynamic variables directly (see code snippet~\ref{code:loggerMsgCalls}). In Java the 3 dots (also known as 'vararg' \cite{Oracle2004}) can be used to achieve such goals (see code snippet~\ref{code:loggerVarArgs}).

\begin{codeCaption}
	\caption{Example message calls to the logger}
	\label{code:loggerMsgCalls}
	\centering
	\begin{lstlisting}[language=Java]
...
\begin{codeCaption}
	\caption{Logger message call with VarArgs}
	\label{code:loggerVarArgs}
	\centering
	\begin{lstlisting}[language=Java]
...
In the event that no configuration file is found a default one is created that includes some guidelines on what options are available (see code snippet~\ref{code:loggerConfigFile}).

\begin{codeCaption}
	\caption{Sample configuration file for the logger}
	\label{code:loggerConfigFile}
	\centering
	\begin{lstlisting}
		//======================================================================================
