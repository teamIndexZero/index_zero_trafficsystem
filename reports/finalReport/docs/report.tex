...
The goal of the project was to create an urban traffic simulation over the connected road network resembling ones found in real world. Such simulation would allow for inspecting different traffic properties such as vehicle throughput in key points and possible effect of changes in rules or road network topology on these properties. In addition to providing basic inspection properties, system would be easily extendable with extra, more complex observable properties such as pollution factors.

Traffic model originated as cellular automata model with map represented as grid consisting of cells which may be either free or occupied by a car. Shortly after intial prototype implementation the real map data requirement was raised. The cellular automata model was found to be inflexible due to fixed size cells which did not pair well with the multitude of forms found in real maps, and had to be abandoned in favour of, or rather morphed into to more physical metric model. In this physical model vehicle bears real-life properties such as speed, acceleration, angle and position on the lane, and moves according to laws of mechanics, which uncovers a potential for further deatlisation of the physical model.
...
While the implementation of the requirements can be named successfull, the accent was shifted from creating complex behavioral models for vehicles such as routing and implementing them as intelligent agents, to a different area of complexity: understanding, representing and displaying real world map data in a format compatible for traffic simulation. Still, vehicles operate in a rather autonomous ways, but they can not be classified as agents, since interface between environment and vehicle is not that well defined, and really cars are only capable of basic reactive behavior.
...
any of those model would not be give expected result.
...
Firstly we could have improved the graphical interface of our program. We might, as our program skirts into a hybrid model, have added
visual detailing, i.e. car sprites, road graphics. Some features, while the zoom factor is changed, could have been respectively removed
or added, for instance: when the zoom factor increases beyond a threshold the dots representing the vehicles are changed to sprites matching
their descriptions. There might have been a point/click interface for adding static map objects to the graph via the GUI, for instance: road block,
construction work or broken down cars.  The GUI map editor could have shown easy OSM error corrections or even synthetic map creation.

The second group of possible improvements are those related to the simulator. Complex traffic lights with not only green and red
but also early/late yellow states could have been added to the program. We thought also about intelligent traffic lights depended on the amounts of the cars
that are waiting at a junction. We set a number of cars that are produced for the simulation purpose (currently 500), but in general we could have improved it
by creating a vehicle generator that depends on the map size. There is also a possibility of vehicles specification based on real-life data. Another project
enlargement could have contained capability for adding third party development of behaviour and metric recorders. If we had had more time we would have even
created a pollution profiler with a visualised heat map. Such a map would be based on the vehicle specifications and numbers.

The maps could be read from other sources, for example Google or Bing maps. The Simulation recorder on the simulator side might have saved large simulations to
files and a have played them on the GUI side.

Logger might have coloured the output callable from the GUI. It's messages might have been filtered, basing on the message's origin or
 granular individual log level control.

\underline{GUI}

\begin{itemize}
	\item As the design skirts into a hybrid model, visual detailing such as car sprites, road graphics, etc... could ostensibly be added and removed dynamically as the zoom factor on the map is changed. I.e.: when the zoom factor is increases beyond a threshold the dots representing the vehicles are changed to sprites matching their descriptions.
	\item A point/click interface for adding static map objects to the graph via the GUI such as road block, construction work or broken down cars,
	\item A GUI map editor for easy OSM error corrections or synthetic map creation,
\end{itemize}

\underline{Simulator}
\begin{itemize}
	\item Traffic-light integration
	\item Vehicle production on the graph scaled to the size of the map instead of a hard limit (currently 500),
	\item Specification of the vehicles produced based on real-life data,
	\item Plug-in capability so that 3rd party development of behaviour and metric recorders can be added easily,
	\item Pollution profiler with heat map visualisation for the GUI based on the vehicle specifications and numbers,
\end{itemize}

\underline{Miscellaneous}
\begin{itemize}
	\item Import from other sources such as Google Maps,
	\item Simulation recorder on the simulator side to save large simulations to files and a player on the GUI side to read those files.
\end{itemize}
